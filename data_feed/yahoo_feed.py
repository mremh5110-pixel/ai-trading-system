...
# ملاحظة: XAUUSD=X غير موجود في Yahoo Finance فعليًا - الرمز الصحيح للذهب هو
# GC=F (عقود الذهب الآجلة COMEX)، وسعرها قريب جدًا من السعر الفوري الحقيقي.
YAHOO_SYMBOL_MAP = {
    "XAUUSD": "GC=F",
    "EURUSD": "EURUSD=X",
    "GBPUSD": "GBPUSD=X",
...
def fetch_multi_timeframe(symbol: str, timeframes: list, bars: int = 500):
    """يرجع (data_by_tf, errors) - errors قاموس بأسباب الفشل الفعلية لكل فريم فشل،
    عشان يظهر السبب الحقيقي في الواجهة بدل رسالة عامة."""
    result, errors = {}, {}
